...
StockPiler3.SkillUp = StockPiler3.SkillUp or {}
...
local function Caps()
    return StockPiler3.TradeSkillCaps
end
...
function SkillUp.MaybeNotifyStall()
    ...
    if StockPiler3.T then
        msg = StockPiler3.T("skillup.stall." .. reason)
    else
...
--- Full SkillUp diagnostic dump (/sp3 skillplan).
...
function SkillUp.ClearRates()
    ...
    SkillUp._pendingApo = nil
    if StockPiler3.Debug and StockPiler3.Debug.LogOp then
        StockPiler3.Debug.LogOp("skillup", "rates cleared")
    end
    return true
end
...
local function ResolveSeedUidForPlantItem(item, plantUid)
    ...
    local entry = StockPiler3.Account and StockPiler3.Account.refines
        and StockPiler3.Account.refines[tostring(plantUid)]
    ...
    if type(recipe) ~= "table" then
        ...
        if not quiet then
            if why == "need_resin" or why == "unstable" then
                if StockPiler3.Refine and StockPiler3.Refine.MarkRefineDue then
                    StockPiler3.Refine.MarkRefineDue("skill-up-resin")
                end
            end
...
local function TOr(key, fallback)
    local Locale = StockPiler3.Locale
    if Locale and Locale.ResolveTemplate then
        local s = Locale.ResolveTemplate(key)
        if s ~= nil then
            return s
        end
    elseif StockPiler3.T then
        local s = StockPiler3.T(key)
        if s ~= nil then
            return s
        end
    end
...
local function TFmt(key, tokens, fallback)
    if StockPiler3.T then
        local s = StockPiler3.T(key, tokens)
        if s ~= nil then
            return s
        end
    end
