...
local function MakeInAddonPerf()
    ...
    function Perf.OnFrame(_timeElapsed)
        ...
        if empty then
            ...
            if Perf._emptyRate <= 3 or (Perf._emptyRate % 20) == 0 then
                StockPiler3.Debug.LogAlways(string.format(
    ...
    function Perf.PrintSummary()
        StockPiler3.Debug.Print(towstring(string.format(
...
if LibPerf and type(LibPerf.Scope) == "function" then
    local Perf = LibPerf.Scope("StockPiler3")
    Perf.Available = true
    function Perf.OnFrame(_timeElapsed)
    end
    local thr = 0
    ...
    StockPiler3.Perf = Perf
else
    StockPiler3.Perf = MakeInAddonPerf()
end
