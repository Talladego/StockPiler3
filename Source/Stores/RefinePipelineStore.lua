...
StockPiler3.RefinePipeline = StockPiler3.RefinePipeline or {}
...
function RP.MarkPending(seedUid, plantUid)
    ...
    local row = RP._outstanding[seedUid]
    if type(row) ~= "table" then
        row = { count = 0, plantUid = plantUid, at = NowSec(), softTried = false }
            ...
            if n > 0 then
                ...
                elseif (now - at) >= ttl then
                    if row.softTried ~= true then
                        row.softTried = true
                        if StockPiler3.Inventory and StockPiler3.Inventory.MarkDirty then
                            StockPiler3.Inventory.MarkDirty({ reason = "refine-expire" })
                        end
                        changed = true
                    else
                        RP._outstanding[seedUid] = nil
